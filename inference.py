...
def parse_args():
    ...
    parser.add_argument("--image-path", type=str, default="demo/image.jpg", help="Path to the input image")
...
def main(params):
    ...
    segmentation_image.save(f"./demo/{filename[:-4]}_mask.png")
    ...
    if params.save_overlay:
        ...
        cv2.imwrite(f"./demo/{filename[:-4]}_mask_color.png", colored_class_map)
        cv2.imwrite(f"./demo/{filename[:-4]}_overlay.png", overlayed_image, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
